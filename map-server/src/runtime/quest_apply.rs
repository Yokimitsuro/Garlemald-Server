...
pub(crate) async fn apply_despawn_actor(
    ...
    // at 06:50:36.94x and 08:07:42.57x vs the surviving load). The
    // ghost-circle problem is solved at the source instead: content
    // areas must not spawn circle-bearing props (SimpleContent30079's
    // stopper dropped — Limsa's 30002, the upstream-tested twin,
    // never had one).
