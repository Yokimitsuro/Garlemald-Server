...
impl WorldManager {
    ...
    pub async fn send_zone_in_bundle(
        ...
        // SEQ-005 content-warp region: ship the parent region UNMODIFIED (106),
        // matching pmeteor. The previous high-16 tag (106 | 0x10000 = 0x1006A)
        // forced a same-zone scene reload (the client's SetMap reload-decision
        // compares the full 32-bit region vs the resident [+0x94]; a same
        // region early-returns → "Now Loading" hang), but it registered the
        // player into resident region 0x1006A — a value the post-cinematic
        // movement + active-mode(F) unlock path doesn't recognise, so the
        // player stayed frozen and F was dead. pmeteor sends plain 106 for this
        // same warp and still reloads, because it sends the instance-entry
        // signal (0x166 "you have entered an instance") in the warp burst —
        // which is the real same-region reload trigger. garlemald never sent
        // 0x166; it now does (apply_do_zone_change_content), so the tag is no
        // longer needed and is removed here to keep the resident region valid.
        // (Garlemald-Server #28.)
