...
impl WorldManager {
    ...
    pub async fn send_zone_in_bundle(
        ...
        // The decompiled client (MapLayoutElement vtable slot 9, FUN_0059ced0)
        // reloads on a SetMap iff region != 0 AND (force-latch [+0xbc] != 0 OR
        // region != resident [+0x94]). The content region equals the resident
        // region (both 106), so the reload depends on the resident [+0x94]
        // being cleared to 0 first by the client's map-element teardown
        // (FUN_0059d0d0, driven by the warp's actor-wipe). The old high-16 tag
        // (106 | 0x10000 = 0x1006A) forced the reload via the region-inequality
        // branch, but committed an INVALID resident region (0x1006A) that broke
        // the post-cinematic movement / active-mode-F unlock. We now ship valid
        // region 106 and rely on the teardown + the inter-packet frame gap in
        // apply_do_zone_change_content to clear [+0x94] before SetMap arrives.
        // (Garlemald-Server #28.)
